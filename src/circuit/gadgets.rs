pub mod gadget {
    ...
    use crate::poseidon::{WIDTH_3, POSEIDON_HASH_PARAM_BLS12_377_SCALAR_ARITY2};
    ...
    pub fn white_list_gadget<CP: CircuitParameters>(
        ...
        let crh_point = bad_hash_to_curve_gadget::<CP>(
            composer,
            &vec![owner_address, token_address],
            &vec![],
        );
        ...
        let merkle_path = MerklePath::from_path(vec![
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(private_inputs[0]),true),
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(private_inputs[1]),false),
            // etc.
        ]);
        let cur_leaf = Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(private_inputs[0]); // node owner address
        let poseidon_hash_param_bls12_377_scalar_arity2 = PoseidonConstants::generate::<WIDTH_3>();
        ...
        let root = merkle_tree_gadget::<CP::CurveScalarField, CP::InnerCurve, PoseidonConstants<CP::CurveScalarField>>(
            composer,
            &commitment,
...
pub mod tests {
    use crate::{circuit::gadgets::gadget::white_list_gadget, note::Note};
    ...
    fn test_white_list_gadget() {
        ...
        // a note owned by a white list user
        let n = Note::<CP>::new();
        let owner_address = white_list[1];
        let token_address = <CP as CircuitParameters>::CurveScalarField::rand(&mut rng);

        let merkle_path = MerklePath::from_path(vec![
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(white_list[0]),true),
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(white_list[1]),true),
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(white_list[2]),true),
            (Node::<CP::CurveScalarField, PoseidonConstants<CP::CurveScalarField>>::new(white_list[3]),true),
        ]);
        ...
        let mut composer = StandardComposer::<
            ...
            <CP as CircuitParameters>::InnerCurve,
        >::new();
        white_list_gadget::<CP>(&mut composer, &vec![a, b], &vec![c]);
        composer.check_circuit_satisfied();
    }
}
