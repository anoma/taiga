...
* The `hash-to-field` function in [`src/lib.rs`](https://github.com/heliaxdev/pbc-playground/blob/master/src/lib.rs) is probably not the right way for hashing into a field.
* The `hash-to-curve` algorithm used in the `commitment` method in [`src/note.rs`](https://github.com/heliaxdev/pbc-playground/blob/master/src/note.rs) is a naive hash that must not be used in a real-world implementation.
* The nullifier computed in the `spend` method in [`src/user.rs`](https://github.com/heliaxdev/pbc-playground/blob/master/src/user.rs) should be replaced by an implementation as specified by Zcash.
