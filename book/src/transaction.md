...
* one `RecVP` proof and one `TokenVP` proof corresponding to an output note,
...
In this current implementation, we set `NUM_NOTE=4` for the number of input and output notes. Moreover, for we are interested in the `SendVP` and the `TokenVP` of input notes and `RecVP` and `TokenVP` of output notes. Finally, every validity predicate is blinded as described [here](blinding.md), leading to a blinding proof.
...
* Four `RecVP` proofs corresponding to the four output note owner constraints,
